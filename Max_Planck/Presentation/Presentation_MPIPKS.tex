...
\begin{alignat*}{2}
H_0 \;\Rightarrow&& \; G_0 =&\; [\omega + i \eta - H_0]^{-1} \\
H_0 + H_1 \;\Rightarrow&& \;G =&\; [\omega - \underbrace{(H_0 + \Sigma)}_{H_\mathrm{eff}}]^{-1}
\end{alignat*}
\pause
...
\vspace{0.5em}
\pause
Take FQH system on a Torus of $L_x \times L_y$ and send $L_x \to 0$
...
\begin{frame}
	...
	\begin{figure}[htp!]	 
			...
			\caption*{ED data for energy gaps (left) and particle entanglement spectrum (right). \blfootnote{B. Michen, C. Repellin, and J.C. Budich,
\href{https://journals.aps.org/prresearch/abstract/10.1103/PhysRevResearch.5.023100}{Phys. Rev. Res. {\bfseries{5}} (2), 023100 (2023)}}}
	\end{figure}
\end{frame}
...
\pause
\begin{itemize}[<+->]
	\item Stabilizes two ground states with opposite parity
	\item Symmetry-protected by $\mathbb Z_2$ wire parity $P_a = (-1)^{N_a}$
...
\vspace{0.5em}
Break parity symmetry by interwire tunneling with flux $\phi$ to create energy gap 
\begin{align}
...
\end{align}
\pause
$\Rightarrow$ Optimal scaling of preparation time $\tau_\mathrm{tot}(L)$ by increasing $p(L)$ with system size $L$.
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[htp!]	 
			...
			\caption*{(a) MPSTE data for fidelity achieved with flux term. Linear scaling $\tau_\mathrm{tot} \propto L$.\\		
			 (b) Performance without flux is qualitatively worse, quadratic scaling $\tau_\mathrm{tot} \propto L^2$.} 
	\end{figure}\blfootnote{B. Michen, T. Pokart, and J.C. Budich,
\href{https://journals.aps.org/prb/abstract/10.1103/dqsb-nzx1}{Phys. Rev. B {\bfseries{112}} (4), 045112 (2025)}}
\end{frame}
...
\begin{itemize}[<+->]
\item Term $\propto \gamma$ can destroy topological edge states of $H_F(j,j',t)$ through scattering
...
\end{itemize}
\vspace{0.5em}
$\Rightarrow$ Disorder stabilizes $\sigma_{xy} = e^2 / h$. IQHE in trivial band structure!
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[onlytextwidth,T]
		\begin{column}{.45\textwidth}
			\textbf{\large Distinction from topological Anderson insulator}	\\
			\vspace{0.5em}
\begin{itemize}[<+->]
\item In thermodynamic limit, any disorder $W > 0$ should suffice
\item Continuity equation yields current $\phi_{\bm \delta, \bm j}(t)$ through lattice link
\begin{align}
\frac{\mathrm d}{\mathrm d t} |\psi_{\bm j}|^2 = \sum_{\bm \delta \in \mathrm {NN}} \underbrace{2 \mathrm {Im} [J_{\bm \delta} (t) \psi^*_{\bm j} \psi_{\bm j + \bm \delta}]}_{-\phi_{\bm \delta, \bm j}(t)} \nonumber
\end{align}
...
\end{column}
		...
		\begin{column}{.55\textwidth}
						\flushright
			\begin{figure}[htp!]	 
				\includegraphics[trim={0cm 0cm 0cm 0cm}, width= 1 \linewidth]{Illustrations/transport_W_EF.png} 
			\end{figure}
		\end{column}
	\end{columns}
\blfootnote{B. Michen and J. C. Budich,
...
\begin{itemize}[<+->]
\item In thermodynamic limit, any disorder $W > 0$ should suffice
\item Large window with IQH phase opens immediately
\item distinction from TAI!
\item More stuff
\item this yields an intriguing order of limits problem between localization length and disorder ampolitude $W$!
\end{itemize}
\end{minipage}%
\hfill
\begin{minipage}{0.45\textwidth}
\includegraphics[width=\linewidth]{Illustrations/transport_W_EF.png}
\end{minipage}
...
\mbox{}% Force wrapfigure to start
...
\vspace*{\fill}% Add space to prevent overflow
